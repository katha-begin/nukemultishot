...
def ensure_variables_for_batch_mode():
    ...
    try:
        ...
        # Update PROJ_ROOT and IMG_ROOT knobs if they exist (replace Windows paths with Linux paths)
        # NOTE: The knobs already exist in the .nk file - we just need to update their values on Linux
        import platform
        path_mappings = {
            'V:/': '/mnt/igloo_swa_v/',
            'V:\\': '/mnt/igloo_swa_v/',
        ...
        for key in ['PROJ_ROOT', 'IMG_ROOT']:
            if key in all_knobs:
                ...
                # Knob doesn't exist - try to create from JSON
...
try:
    ...
    if nuke.GUI:
        ...
        # CRITICAL: In batch mode, the script is loaded BEFORE init.py runs!
        # So we need to call the function immediately, not as a callback.
        # The script is already loaded when we get here.

        # Call immediately to create knobs from JSON
        ensure_variables_for_batch_mode()

        # Also add as callbacks for when scripts are loaded (for safety)
